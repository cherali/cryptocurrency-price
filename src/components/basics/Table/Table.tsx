...
// TODO: add pagination
const Table = <T extends object>({ columns, data, showTableNumber = false, loading = false, extendBody, cellKey }: TableProps<T>) => {
	return (
		<div>
			{loading && <Loading />}
				...
				<>
					<TableStyle>
						...
						<TableBody>
							...
							{data.map((row, index) => (
								<tr key={cellKey ? (row[cellKey] as string) : index}>
